...
alias nv=nvim
alias solsta_login_dev='export SOLSTA_ACCESS_TOKEN=$(solsta_cli login prompt --org=solsta --out=json,minify --display_token=true --stage=dev | jq -Rr "fromjson? | select(.type == \"STOP\") | .accessToken")'
...
alias solsta_login_show='echo $SOLSTA_ACCESS_TOKEN | cut -d. -f2 | base64 -d 2>/dev/null | jq .'
